 ...
 */
var baseSetToString = !uiStore.defineProperty ? identity : function(func, string) {
  return uiStore.defineProperty(func, 'toString', {
    'configurable': true,
    'enumerable': false,
...
function isIterateeCall(value, index, object) {
  if (!uiStore.isObject(object)) {
    return false;
  }
  var type = typeof index;
  if (type == 'number'
        ? (uiStore.isArrayLike(object) && uiStore.isIndex(index, object.length))
        : (type == 'string' && index in object)
      ) {
    return uiStore.eq(object[index], value);
  }
  return false;
}
...
function isPlainObject(value) {
  if (!uiStore.isObjectLike(value) || uiStore.baseGetTag(value) != objectTag) {
    return false;
  }
  var proto = uiStore.getPrototype(value);
  if (proto === null) {
    return true;
  }
...
function assignMergeValue(object, key, value) {
  if ((value !== undefined && !uiStore.eq(object[key], value)) ||
      (value === undefined && !(key in object))) {
    uiStore.baseAssignValue(object, key, value);
  }
}
...
function isArrayLikeObject(value) {
  return uiStore.isObjectLike(value) && uiStore.isArrayLike(value);
}
...
function toPlainObject(value) {
  return uiStore.copyObject(value, uiStore.keysIn(value));
}
...
function baseMergeDeep(object, source, key, srcIndex, mergeFunc, customizer, stack) {
  ...
  if (isCommon) {
    var isArr = uiStore.isArray(srcValue),
        isBuff = !isArr && uiStore.isBuffer(srcValue),
        isTyped = !isArr && !isBuff && uiStore.isTypedArray(srcValue);

    newValue = srcValue;
    if (isArr || isBuff || isTyped) {
      if (uiStore.isArray(objValue)) {
        newValue = objValue;
      }
      else if (isArrayLikeObject(objValue)) {
        newValue = uiStore.copyArray(objValue);
      }
      else if (isBuff) {
        isCommon = false;
        newValue = uiStore.cloneBuffer(srcValue, true);
      }
      else if (isTyped) {
        isCommon = false;
        newValue = uiStore.cloneTypedArray(srcValue, true);
      }
      else {
        newValue = [];
      }
    }
    else if (isPlainObject(srcValue) || uiStore.isArguments(srcValue)) {
      newValue = objValue;
      if (uiStore.isArguments(objValue)) {
        newValue = toPlainObject(objValue);
      }
      else if (!uiStore.isObject(objValue) || uiStore.isFunction(objValue)) {
        newValue = uiStore.initCloneObject(srcValue);
      }
    }
    else {
      isCommon = false;
...
function baseMerge(object, source, srcIndex, customizer, stack) {
  ...
  baseFor$1(source, function(srcValue, key) {
    stack || (stack = new uiStore.Stack);
    if (uiStore.isObject(srcValue)) {
