...
<div class="sm:flex sm:items-center sm:justify-between">
    <div>
        <h1 class="text-2xl font-semibold leading-6 text-gray-900">编辑文档</h1>
        <p class="mt-2 text-sm text-gray-700">修改文档的基本信息和属性。</p>
    </div>
    <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none space-x-2">
        <a href="{{ route('admin.documents.show', $document) }}" class="btn-secondary">查看详情</a>
...
<div class="max-w-4xl mx-auto">
    <form method="POST" action="{{ route('admin.documents.update', $document) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">当前文档信息</h3>
                <div class="bg-gray-50 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <div class="text-sm text-gray-500">{{ number_format($document->file_size / 1024 / 1024, 2) }} MB | 上传于 {{ $document->created_at->format('Y-m-d H:i') }}</div>
                        </div>
                        <div class="ml-auto">
                            <a href="{{ route('admin.documents.download', $document) }}" class="text-primary-600 hover:text-primary-900 text-sm">下载文件</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="document" {{ old('type', $document->type) == 'document' ? 'selected' : '' }}>文档</option>
                            <option value="video" {{ old('type', $document->type) == 'video' ? 'selected' : '' }}>视频</option>
                            <option value="audio" {{ old('type', $document->type) == 'audio' ? 'selected' : '' }}>音频</option>
                        </select>
                        @error('type')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="status" 
                            ...
                            <option value="1" {{ old('status', $document->status) == '1' ? 'selected' : '' }}>启用</option>
                            <option value="0" {{ old('status', $document->status) == '0' ? 'selected' : '' }}>禁用</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="description" 
                                  ...
                                  placeholder="请输入文档描述...">{{ old('description', $document->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">文件替换（可选）</h3>
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-yellow-700">
                                <ul class="list-disc list-inside space-y-1">
                                    <li>如果不选择新文件，将保留原文件</li>
                                    <li>上传新文件会覆盖原文件且无法恢复</li>
                                    <li>建议新文件与原文件格式保持一致</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <div>
                    <label for="file" class="block text-sm font-medium text-gray-700">选择新文件（可选）</label>
                        ...
                        <div class="space-y-1 text-center w-full">
                            ...
                            <div id="file-uploaded-info" class="hidden mt-3">
                                <div class="flex items-center space-x-2 bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-700">
                                    ...
                                    <span class="font-medium">新文件已上传成功</span>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <div>
                    ...
                    @php
                        $oldTags = old('tags', $document->tags->pluck('id')->toArray());
                    @endphp
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <div>
                    ...
                    @php
                        $oldVisibleRoles = old('visible_roles', $document->visible_roles ?? []);
                    @endphp
                    ...
                    @if($document->visible_roles && !empty($document->visible_roles))
                        <div class="mt-3 text-sm text-gray-600">
                            <strong>当前设置：</strong>{{ $document->visible_roles_text }}
                        </div>
                    @else
                        <div class="mt-3 text-sm text-gray-600">
                            <strong>当前设置：</strong>所有人可见
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">修改记录</h3>
                
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">创建时间</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $document->created_at->format('Y-m-d H:i:s') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">最后修改</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $document->updated_at->format('Y-m-d H:i:s') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('admin.documents.show', $document) }}" class="btn-secondary">取消</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        uploadBtn.addEventListener('click', function() {
            if (selectedFile) {
                if (confirm('您确定要上传新文件替换原文件吗？此操作无法恢复。')) {
        ...
        window.clearFile = function() {
            ...
            document.getElementById('temp_file_path').value = '';
            document.getElementById('original_name').value = '';
            document.getElementById('file_size').value = '';
            document.getElementById('mime_type').value = '';
